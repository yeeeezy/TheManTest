...
FEAT-080 起，MaintenanceWorker 的三把初始枪均为 `AFirearm` Blueprint 配置：`BP_RepairGun`、`BP_ElectricGun`、`BP_ExplosionGun`。电击枪使用 `SM_ElectricGun + NS_ElectricGun_LaserMuzzle + NS_ElectricGun_LaserImpact`；爆炸枪使用 `SM_ExplosionGun + NS_ExplosionGun_Muzzle + NS_ExplosionGun_Impact`。两把新枪的玩法数据复制 RepairGun，但动画、AnimBP、音频、CameraShake、Bullet、GameplayCue、弹体 Mesh/材质以及迁入的模型/VFX 依赖均位于各自 `/Game/Weapons/<WeaponName>/`，不得重新指回 RepairGun 专属资产。
