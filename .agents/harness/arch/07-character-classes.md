...
2026-09-14返修：大厅实际使用三把正式武器，WeaponPresentations按0维修枪／1爆破枪／2电击枪配置，实例DisplayWeaponIndex或SetDisplayWeaponIndex选择，非法运行时索引忽略。每项FLobbyWeaponPresentation持有WeaponClass、RelaxedAttachment／ReadyAttachment和RelaxedLeftGrip／ReadyLeftGrip；挂点相对右手／插槽，左手握点相对武器Actor原点。只读武器CDO的实际模型、材质与组件变换，骨骼模型优先，与正式Equip一致。新增无碰撞DisplaySkeletalWeapon显示维修枪SK_SCFRIFLE及其三槽材质；DisplayWeapon显示另外两把静态枪，任一时刻仅显示其中一个，Standing均隐藏。旧95.3cm维修枪静态代理不能代表正式76.8cm骨骼枪模，不再使用原配Rifle_01替代正式三枪。

ULobbyCharacterAnimInstance继承UAnimSingleNodeInstance，原生代理先评估原待机，再用同帧右手姿势和选中握点求解左臂TwoBoneIK、设置手腕朝向。左臂节长不变，不拉伸，不改右臂／身体／原动画资产。FBoneReference负责从骨骼映射到CompactPose，不能把Mesh骨骼索引直接当成SkeletonPose索引。自定义右手Socket包含其父骨骼和局部变换。Standing或隐藏放松武器时不施加握持IK。此为正常运行时IK，不是动画重定向，没有新增源骨架、IKRig或Retargeter。AnimationCore为私有运行时依赖。
