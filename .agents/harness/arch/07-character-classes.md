...
2026-09-14返修：大厅实际使用三把正式武器，WeaponPresentations按0维修枪／1爆破枪／2电击枪配置，实例DisplayWeaponIndex或SetDisplayWeaponIndex选择，非法运行时索引忽略。每项FLobbyWeaponPresentation持有WeaponClass、RelaxedAttachment／ReadyAttachment、RelaxedAnimations与ReadyAnimation；挂点相对右手／插槽，每枪独立配置两条放松及一条举枪成品动画。只读武器CDO的实际模型、材质与组件变换，骨骼模型优先，与正式Equip一致。新增无碰撞DisplaySkeletalWeapon显示维修枪SK_SCFRIFLE及其三槽材质；DisplayWeapon显示另外两把静态枪，任一时刻仅显示其中一个，Standing均隐藏。旧95.3cm维修枪静态代理不能代表正式76.8cm骨骼枪模，不再使用原配Rifle_01替代正式三枪。

大厅已删除ULobbyCharacterAnimInstance及全部左手目标／IK逻辑，使用OverrideAnimationData／PlayAnimation直接播放每枪成品。RelaxedIdleIndex／SetRelaxedIdleIndex(0/1)选择并进入放松版本；换枪时索引钳制，无效运行时版本忽略。仅hand_l旋转轨道在TMIIR外部制作；上臂、前臂、手腕位置和其他原动画轨道保持。九条新增成品为AS_MaintenanceWorker_Lobby_{RepairGun,ExplosionGun,ElectricGun}_{RelaxedIdle,RelaxedIdle_02,RifleIdle}；原五条动画保留，Standing仍用原空手动画。
