...
| `Source/TheManTest/Private/Characters/CharacterBase/FPSCharacterBase/FPSCharacterBase.cpp` | 组件挂载：**HeadCamera←RootComponent(capsule)**，**ViewmodelRoot←HeadCamera，ArmsViewMesh←ViewmodelRoot**。PIE 实例中的 `ViewmodelOffsetLocation` / `BaseArmsRotation` 是 Arms 静态构图权威值，Tick 每帧无条件写入 `ArmsViewMesh`，不做变化检测；`ViewmodelRoot` 只叠加冲刺动态 Pitch。ArmsViewMesh 独立求值，GetMesh() 在 Body AnimBP 内合成其上半身 Pose。`ShadowBodyMesh` / `ShadowUpperBodyMesh` 为弃用空组件；唯一完整影子来自 `GetMesh()` 的 CastHiddenShadow。|
