...
| `Source/TheManTest/Private/Characters/CharacterBase/FPSCharacterBase/FPSCharacterBase.cpp` | 组件挂载：**HeadCamera←RootComponent(capsule)，相对 Z≈+77 固定眼高，默认 FOV=110，bUsePawnControlRotation**；**ViewmodelRoot←HeadCamera，ArmsViewMesh←ViewmodelRoot**。FEAT-074 中鼠标旋转滞后与自创方向偏移已停用；C++ 按 VFXPack Walking/Running 参数生成轻微旋转波形并仅写入 ViewmodelRoot，HeadCamera/弹道不变。`GetMesh()` 继续驱动身体 locomotion；MaintenanceWorker 特例中 `ShadowBodyMesh` Leader 改为 `ArmsViewMesh`，使投影上半身与 VFXPack 持枪 Pose 同源，`LegsMesh` 仍 Leader=`GetMesh()`。BodyRoot 每帧保持 Actor Yaw/直立。|
