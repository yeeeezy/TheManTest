...
| `Source/TheManTest/Private/Characters/CharacterBase/FPSCharacterBase/FPSCharacterBase.cpp` | 组件挂载：**HeadCamera←RootComponent(capsule)**，**ViewmodelRoot←HeadCamera，ArmsViewMesh←ViewmodelRoot**。`ArmsViewMesh` 构造默认构图为 Location `(-6.330288,-6.449130,-141.685038)`、Rotation `(Roll=0.581459,Pitch=0.297436,Yaw=-117.090375)`；C++ 不在 BeginPlay/Tick 覆盖该静态 Transform，具体角色 BP 仍可覆盖。`ViewmodelRoot` 保持零 Location，Tick 只写实际速度驱动的冲刺 Pitch；不用它调静态构图。ArmsViewMesh 独立求值，GetMesh() 在 Body AnimBP 内合成其上半身 Pose。`ShadowBodyMesh` / `ShadowUpperBodyMesh` 为弃用空组件；唯一完整影子来自 `GetMesh()` 的 CastHiddenShadow。|
