...
| `Source/TheManTest/Private/Characters/CharacterBase/FPSCharacterBase/FPSCharacterBase.cpp` | 组件挂载：**HeadCamera←RootComponent(capsule)，相对 Z≈+77 固定眼高，bUsePawnControlRotation**；**ViewmodelRoot←HeadCamera，ArmsViewMesh←ViewmodelRoot**。FEAT-074 最终使用 VFXPack 77° FOV；鼠标旋转滞后保持停用。C++ 同步驱动 ArmsViewMesh 与 GetMesh 的原版 VFXPack AnimBP，并复刻 Body_Sway 的移动参数：Side Clamp `[-1,1]`，Walk/Sprint 插值速度 2/8，写入 `Lean_Sides_Amount`；不叠加 MouseX。`ShadowBodyMesh` 与 `LegsMesh` 均 Leader=`GetMesh()`。BodyRoot 每帧保持 Actor Yaw/直立。|
