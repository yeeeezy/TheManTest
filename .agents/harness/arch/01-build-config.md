...
- FEAT-082：已移除大厅IK代理及其AnimationCore私有依赖，使用引擎标准UAnimSingleNodeInstance直接播放成品动画。
...
- FEAT-082展示菜单：Slate和SlateCore改为Private运行时依赖，供UMG按钮样式；UMGEditor仅Editor依赖，供显式创建可编辑WidgetBlueprint的空布局初始化助手。
