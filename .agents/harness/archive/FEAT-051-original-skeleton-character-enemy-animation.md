...
## Shooter Jump Blend Alignment

Session109 compared the actual FPSShooter1 animation blueprints rather than assuming the Rifle-folder jump clips were active. `ABP_TP_Rifle`, `ABP_TP_Pistol`, and `ABP_Unarmed` all reference the same Unarmed `MM_Jump`, `MM_Fall_Loop`, and `MM_Land` set that supplied the accepted retargets; the unused Rifle jump assets have no blueprint referrers. The remaining feel difference was therefore in transition configuration, not animation selection.

The Shooter rifle state machine uses Linear transitions, with its shortest jump transitions at 0.1 seconds. TheManTest had normalized every locomotion transition to a 0.2-second Hermite curve, including `AnimStateTransitionNode_10`, the `To Land -> Jump_End` entry that had previously been changed from zero. That longer eased blend delays the visible landing pose after physical contact. Only this landing-entry transition was aligned to Shooter behavior: `Linear`, 0.1 seconds. The 0.2-second exits from `Jump_End` to grounded locomotion remain unchanged so recovery still settles smoothly.

Unreal MCP recompiled and saved `TABP_BodyLocomotion` and `ABP_MaintenanceWorker`, then read back `LINEAR / 0.1`. PIE delivered two SpaceBar jumps to `BP_MaintenanceWorker_C_0`, returned to the normal equipped grounded pose after both, and captured `JumpBlend_ShooterAligned_AfterTwoJumps.png`; no Blueprint compile/load error or disappearance was observed. Final subjective timing remains for the user's foreground play test.

## Default First-Person FOV

Session110 set the shared `AFPSCharacterBase::HeadCamera` default field of view to 110 degrees. The constructor establishes the component default and `BeginPlay` reapplies 110 so existing character Blueprint component templates cannot retain an older serialized value. Live Coding succeeded in the already-open editor. PIE runtime inspection of `BP_MaintenanceWorker_C_0.HeadCamera` read back `FieldOfView=110.0`.

Session111 stopped before implementing camera-position tuning. The agreed next-pass structure remains `Capsule -> HeadCamera -> ViewmodelRoot -> ArmsViewMesh`; no SpringArm will be added. Camera offset belongs on `HeadCamera`, while the visible arms/weapon composition is compensated and tuned on `ViewmodelRoot`. `ArmsViewMesh` remains reserved for imported-model axis/origin correction. The current `BP_MaintenanceWorker.uasset` is dirty and may contain the user's editor-side adjustments, so it must not be overwritten or checkpointed blindly next session.
