...
- [x] 修复切入 RepairGun 时手臂/枪在最终持枪位置闪帧：根因是 Linked Anim Layer 先评估到 Idle，而此前仅隐藏新枪、未隐藏 `ArmsViewMesh`。有 Montage 的切换现于准备阶段收起旧枪并隐藏整套第一人称 viewmodel，Montage 起始姿势完成动画评估后同帧显示手臂与新枪；`CharacterMesh0`/影子持续可见和更新。暂停 PIE 证据为准备态 `arms=false, weapon hidden=true`，首个可见态 `arms=true, weapon hidden=false, Montage=0.3333s`；快速 4 连切、W=250 与 Shadow Leader 回归通过。
...
## Session99 handoff - FEAT-051 active (2026-07-27)
...
- 用户澄清先前现象不是空手，而是 RepairGun 从底部抬起前，手臂和枪先在最终持枪位置闪几帧。根因是切换同帧链接的新武器层先输出最终 Idle，旧方案仅隐藏新枪，`ArmsViewMesh` 仍可见。
- Montage 切换现在收起旧装备，并在准备阶段同时隐藏 `ArmsViewMesh` 与新装备；播放 Montage 后再等一次动画评估，在同一回调显示手臂和新枪。全身 `CharacterMesh0` 从不隐藏，因此影子仍连续播放装备动画。
- 暂停 PIE 状态验证：切回准备态 `ArmsViewMesh=false`、Repair/Test 均 hidden；两次步进后的首个可见态 `ArmsViewMesh=true`、Repair hidden=false、Montage position=0.3333334。截图 `EquipViewmodel_FirstVisible.png`。快速 4 连切最终恢复 Repair 可见且手臂可见；W 移动速度 250，`ShadowBodyMesh` Leader 仍为 `CharacterMesh0`。
