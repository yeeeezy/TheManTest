...
3. 阅读 `feature_list.json`，确认当前活跃功能及其完成标准。该文件只保留仍需关注的功能；已完成历史位于 `feature_archive.json`，启动时无需读取。
...
- **完成项移出启动索引。** 功能状态改为 `done` 时，将其完整条目从 `feature_list.json` 移入 `feature_archive.json`；两个 JSON 中的功能 ID 必须全局唯一。需要查询已完成历史时按 ID 定向读取归档索引，不要在启动时加载整份历史。
...
- **源码与 Content 使用同一所有权原则。** 角色专属代码进入 `Characters/<Character>/`，Enemy 专属代码进入对应 Enemy，Weapon 专属代码进入对应 Weapon；不得把所有 Gameplay Ability 长期堆在全局 `GAS/Abilities`。
- **专属优先，`_Shared` 次之。** 只有多个具体对象实际复用的代码才放最近共同语义根的 `_Shared`。全项目公共基础设施放 `Core/_Shared`，不能用 `_Shared` 掩盖归属不清。
- **移动源码必须同步维护。** 更新所有 include、架构文档、测试和硬编码资产路径，并执行 Development Editor / Win64 冷编译；涉及资产路径时还要执行冷启动加载与引用验证。

## 项目框架地图（按需读取）

### 音效强制接入规则

- 新增或替换任何音效前必须读取`arch/14-audio-policy.md`。一次性玩法音效通过Sound Cue封装轻微随机音高/音量，多素材使用随机选择；调用端禁止再次随机。UI/对白/扫描识别/循环音等例外必须记录。
- 按用途选择衰减和并发资产，不能把所有音效套同一个传播范围；衰减资产不负责随机。新增声音必须补充消费者引用与验证清单，并检查实际PIE播放。
- 子弹命中角色只由角色Hit Cue发声，武器Impact仅在环境发声；这条分工不改变粒子、血迹或伤害。
...
### 敌人伤害强度倍率（用户要求，2026-09-13）

- 后续新增或迁入任何敌人伤害技能，都必须接入 `AEnemyBase::GetDamageMultiplier()`：最终伤害 = 基础伤害 × 当前强度倍率，并且只乘一次。不能只接阶段技能集而遗漏伤害成长。
- 战斗阶段只选择技能集；强度倍率来自回合／增强波次，不得把阶段号直接作为伤害倍率。形态切换不重置或重复叠加倍率。
- 弹体伤害在生成时取倍率；直接命中／范围伤害在结算时取倍率，同一次范围结算对所有目标使用同一份数值。不得改写技能基础伤害导致逐次累乘，Cue 不参与伤害计算。
- 验证至少覆盖基础倍率、增强倍率及上限，范围伤害继续按 ASC 去重。设计上需要固定伤害的例外应明确记录，不能静默漏接。
