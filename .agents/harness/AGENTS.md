...
- **功能归属优先，资产类型次之。** 专属资源放在所属功能目录下，再使用 `Effects/Materials`、`Effects/Textures`、`Effects/Functions`、`Effects/Meshes` 等类型子目录；确有两个以上系统共用的资源才放 `/Game/Effects/_Shared/` 对应类型目录。
