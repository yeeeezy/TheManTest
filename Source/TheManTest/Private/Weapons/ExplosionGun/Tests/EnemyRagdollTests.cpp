...
public:
 ...
 bool Update() override
 {
  ...
  if(Stage==1)
  {
   ...
   if(Case<3)
   {
    auto* Class=LoadClass<ABulletBase>(nullptr,Paths[Case]);
  ...
  if(Stage==2)
  {
   ...
   auto* Shot=W->SpawnActor<ABulletBase>(LoadClass<ABulletBase>(nullptr,Paths[Case]),Target-FVector(150,0,0),FRotator::ZeroRotator);Flying=Shot;
   Shot->Damage=0;
   if(auto* Explosive=Cast<AExplosionGunBullet>(Shot)){Explosive->ExplosionDelay=10;Explosive->BulletTime.bEnabled=false;}
