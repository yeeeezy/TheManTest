...
void AdvanceScorpion(UCoreMorphScorpionCombat* C,float Seconds){for(float Left=Seconds;Left>KINDA_SMALL_NUMBER;Left-=1.f/120)C->TickComponent(FMath::Min(Left,1.f/120),LEVELTICK_All,nullptr);}
...
bool FCheckCoreMorphScorpion::Update()
{
 ...
 ClearCooldown(B);B->UseRandomSkill(Victim,EEnemySkillRange::Near);AdvanceScorpion(C,2.5f);
 ...
 for(float DeathTime:{.3f,1.05f,1.6f})
 {
...
{
 FAutomationTestBase* Test;double Started=0;bool WalkShot=false,WindShot=false,ThrustShot=false;
public:
 ...
 bool Update() override
 {
  ...
  if(!WindShot && C->Action==ECoreMorphScorpionAction::Windup && C->ActionTime>.6f){WindShot=true;Shot(TEXT("Scorpion-Windup.png"));}
  ...
  Test->TestTrue(TEXT("Saved main BT runs complete flight, morph, approach and GA strike"),C->StrikeCount>=1 && WalkShot && WindShot && ThrustShot);
