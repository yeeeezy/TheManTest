...
bool FCheckReassembly::Update()
{
    ...
    Test->TestEqual(TEXT("Flight and transformation are each granted once"),ASC->GetActivatableAbilities().Num(),2);
    ...
    Test->TestEqual(TEXT("No repeated ability grants across cancellations and forms"),ASC->GetActivatableAbilities().Num(),2);
