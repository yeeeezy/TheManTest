...
bool FCoreMorphValidate::Update()
{
	...
	Test->TestEqual(TEXT("One flight and one reassembly ability grant"), ASC->GetActivatableAbilities().Num(), 3);
	...
	Test->TestEqual(TEXT("Phase does not grant flight twice"), ASC->GetActivatableAbilities().Num(), 3);
	ASC->CancelAllAbilities();
...
public:
	...
	bool Update() override
	{
		...
		if (!Started)
		{
			...
			Test->TestEqual(TEXT("Switching routes preserves the two distinct grants"), Boss->GetAbilitySystemComponent()->GetActivatableAbilities().Num(), 3);
